...
def default_compute_score(
    ...
    if type(data_source) != str:
        reslist = []
        if val_type == 'llm':
            # LLM judge only supports batched scoring.
            ...
            for data_source_e, solution_str_e, ground_truth_e in zip(data_source, solution_str, ground_truth):
                if data_source_e in ["scifact_evidence", "qasper_evidence"]:
                    from . import evidence_reward
                    res = evidence_reward.compute_score(solution_str_e, ground_truth_e, data_source_e, val_type=val_type)
                    reslist.append(res)
                elif data_source_e in ['nq', "2wiki", "Bamboogle", "hotpotqa", "musique", "tq", "popqa", "browse_comp", "browse_comp_zh", "xbench_deepsearch", "hotpot", "zhihu", "webshaper"] or 'browse_comp' in data_source_e:
    ...
    if data_source == "openai/gsm8k":
        ...
        if info_gain_reward is not None and len(info_gain_reward) > 0:
            ...
            res = info_gain.compute_score(solution_str, ground_truth, data_source,
                                          val_type=val_type, info_gain_reward=info_gain_reward,
                                          tokenizer=tokenizer, is_validation=is_valid,
    ...
    elif data_source in ["scifact_evidence", "qasper_evidence"]:
        from . import evidence_reward

        res = evidence_reward.compute_score(solution_str, ground_truth, data_source, val_type=val_type)
    elif data_source in [
        "searchR1_nq",
        "searchR1_triviaqa",
        "searchR1_popqa",
